...
std::basic_string<charT,traits,allocator> buffer;
/* A record in a stream will look like: tag:root:value\n
With _tag_delim = ':' & _record_delim = '\n'
*/
  if (std::getline(_istream, buffer, _record_delim)){
  auto itbegin = std::begin(buffer)
     , itend = std::end(buffer)
     , iter_value = std::find(itbegin, itend, _tag_delim);
  return basic_flag<charT,traits,allocator>(
           itbegin
         , iter_value
         , std::find(++iter_value, itend, _tag_delim)
         , itend);
  }
throw std::runtime_error("Could not determine tag, database stream is not well formatted.");
}
